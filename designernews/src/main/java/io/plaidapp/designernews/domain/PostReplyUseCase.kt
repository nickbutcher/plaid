...
class PostReplyUseCase(
     ...
     */
    suspend operator fun invoke(body: CharSequence, parentCommentId: Long): Result<CommentResponse> {
        checkNotNull(loginRepository.user) {
            "User should be logged in, in order to post a comment"
        }
        val userId = loginRepository.user!!.id
        return commentsRepository.postReply(body, parentCommentId, userId)
    }
}