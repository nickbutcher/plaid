...
/**
 * Modification of {@see android.support.v4.widget.ViewDragHelper} which instead of
 * dragging a view attached to your touch point, has a notion of a maximum drag distance
 * and applies an elasticity up to that point.
 * <p/>
 * ViewDragHelper is a utility class for writing custom ViewGroups. It offers a number
 * of useful operations and state tracking for allowing a user to drag and reposition
 * views within their parent ViewGroup.
 */
public class ElasticViewDragHelper {
    /**
    ...
    private static final String TAG = "FrictionViewDragHelper";
     ...
     */
    private ElasticViewDragHelper(Context context, ViewGroup forParent, Callback cb) {
        if (forParent == null) {
     ...
     */
    public static ElasticViewDragHelper create(ViewGroup forParent, Callback cb) {
        return new ElasticViewDragHelper(forParent.getContext(), forParent, cb);
    }
     ...
     * @return a new ViewDragHelper instance
     */
    public static ElasticViewDragHelper create(ViewGroup forParent, float sensitivity, Callback
            cb) {
        final ElasticViewDragHelper helper = create(forParent, cb);
    ...
    public void processTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mDragState == STATE_DRAGGING) {
                    setLastDragSize(MotionEventCompat.getPointerId(ev, 0));
                    releaseViewForPointerUp();
                }
                cancel();
                break;
    ...
    public int getLastDragSize() {
        return mLastDragSize;
    }

    private void setLastDragSize(int pointerId) {
        // record how large the last drag gesture was in Y where dragging downward is positive
        mLastDragSize = (int) (mLastMotionY[pointerId] - mInitialMotionY[pointerId]);
    }
    ...
    private void dragTo(int left, int top, int dx, int dy) {
        ...
        if (dy != 0) {
            ...
            // this is the essential deviation from ViewDragHelper
            int offset = clampedY - oldTop;
            if (isElastic) {
                offset *= ((float) (mMaxDrag - mCapturedView.getTop()) / mMaxDrag);
            }
    ...
    public boolean isViewUnder(View view, int x, int y) {
        ...
        return x >= (view.getLeft() + view.getTranslationX()) &&
                x < (view.getRight() + view.getTranslationX()) &&
                y >= (view.getTop() + view.getTranslationY()) &&
                y < (view.getBottom() + view.getTranslationY());
    }
